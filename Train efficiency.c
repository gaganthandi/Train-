...
void station_init(struct station *station)
{
  	...
  	pthread_cond_init(&(station->p_seated_cond), NULL);
...
void station_load_train(struct station *station, int count)
{
  	...
  	while ((station->out_p > 0) && (count > 0))
	{
    	pthread_cond_signal(&(station->train_arrived_cond));
  		count--;
  		pthread_cond_wait(&(station->p_seated_cond), &(station->lock));
  	}
	if (station->in_p> 0)
...
void station_wait_for_train(struct station *station)
{
  	...
  	station->in_p++;
  	pthread_mutex_unlock(&(station->lock));
	pthread_cond_signal(&(station->p_seated_cond));
}
//-----------------------------------------------------------------------------------
void station_on_board(struct station *station)
{
...
{
	int i;
	const int total_p = 100;
	int p_left = total_p;
	int total_p_boarded = 0;
	...
	while (p_left > 0)
	{
		...
		int threads_to_reap = MIN(p_left, free_seats);
		...
		p_left -= threads_reaped;
		total_p_boarded += threads_reaped;
		printf("Train departed station with %d new passenger(s) (expected %d)%s\n",threads_to_reap, threads_reaped,(threads_to_reap != threads_reaped) ? " *****" : "");
